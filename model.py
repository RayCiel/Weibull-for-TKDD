...
def get_delta_series(group):  #return time difference by second
    # return Series
...
def main(test=False, user_data_file=None, cascade_file=None, pcascade_file=None, final_file=None, prediction_file=None):
    ...
    # diffusion data
    # root_status_id | root_user_id | user_id | time including the root status itself
    if not test:
        user_data = pd.read_csv(user_data_file)  # DataFrame
        cascade = pd.read_csv(cascade_file,
                              header=0,
                              names=['original_status_id', 'original_user_id', 'user_id', 'time'],
                              parse_dates=['time'],
    ...
    [user_data, cascade, C] = prepare(user_data, cascade)
